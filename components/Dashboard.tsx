...
const StreakDay: React.FC<StreakDayProps> = ({ label, active, isToday }) => (
  <div className="flex flex-col items-center gap-1.5">
    <span
      ...
      style={{ color: isToday ? '#f97316' : 'transparent' }}
    >
      {isToday ? 'Today' : '.'}
    </span>
    <div
    ...
    try {
      ...
      if (isSupabaseConfigured() && userId) {
        ...
        if (!workoutStreak || !workoutStreak.current_streak) {
          const savedWorkouts: WorkoutEntry[] = JSON.parse(
        ...
        if (todayWorkouts.length === 0) {
          const saved: WorkoutEntry[] = JSON.parse(
            localStorage.getItem('fitbridge_manual_workouts') || '[]'
          );
          const base = new Date();
          base.setHours(0, 0, 0, 0);
          const local = saved.filter((w) => {
          ...
          setTodayWorkoutCalsBurned(
            todayWorkouts.reduce((s, w) => s + (w.calories_burned ?? 0), 0)
          );
        }
        ...
        const savedMeals: MealEntry[] = JSON.parse(
          localStorage.getItem('fitbridge_manual_meals') || '[]'
        );
        ...
        const localToday = savedWorkouts.filter(
  ...
  return (
    <div className="pb-40 animate-fade-in min-h-screen" style={{ background: '#080808' }}>
      ...
      <div className="px-5 pt-12 pb-6 flex items-start justify-between">
        <div>
          <p className="text-xs font-semibold text-zinc-500 uppercase tracking-[0.15em] mb-1">
            {greeting}
          </p>
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, #1a0a00 0%, #0f0f0f 60%, #1a0a00 100%)',
      ...
      >
        ...
        <div className="relative z-10 p-5 pb-4">
          <div className="flex items-start justify-between mb-5">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Flame
                  size={18}
                  className={streak > 0 ? 'text-orange-500 fill-orange-500' : 'text-zinc-600'}
                />
                <span className="text-[10px] font-black text-orange-500/70 uppercase tracking-[0.2em]">
                  Day Streak
                </span>
              </div>
            ...
            <div className="flex flex-col items-center gap-2">
              <div
                ...
                style={{ background: 'rgba(0,0,0,0.4)', borderColor: currentLevel.color + '40' }}
              >
              ...
              <div className="w-16">
                ...
                <p className="text-[8px] text-center text-zinc-600 font-bold mt-0.5">
          ...
          <div className="flex justify-between items-end px-1">
            {last7Days.map((day, i) => (
              <StreakDay key={i} label={day.label} active={day.active} isToday={day.isToday} />
            ))}
          </div>
          ...
          {streak > 2 && (
            <div className="mt-4 flex items-center gap-2 bg-black/30 rounded-2xl p-3 border border-white/5">
              ...
              <span className="text-xs font-bold text-zinc-400">
                Streak Shield active — you're protected
              </span>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.15em] mb-0.5">
              Daily Load
            </p>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            >
              <span className="block text-lg font-black leading-none" style={{ color: m.color }}>
                {m.val}
                {m.suffix ?? ''}
              </span>
              <span className="block text-[9px] font-bold text-zinc-500 uppercase tracking-wider mt-0.5">
                {m.label}
              </span>
            </div>
          ))}
      ...
      {todayWorkout ? (
        <div
          className="mx-4 mb-4 rounded-[24px] p-4 flex items-center gap-3"
          style={{
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-black text-blue-500/70 uppercase tracking-widest">
              Today's Workout
            </p>
        ...
        >
          ...
          <button
            ...
            className="text-xs font-black text-blue-500 flex items-center gap-0.5 flex-shrink-0"
          >
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-zinc-600 font-medium">
              {goalsCompleted} of {dailyGoals.length} complete
            </p>
          </div>
          <div className="flex items-center gap-1">
            {dailyGoals.map((g, i) => (
              <div
                ...
                style={{ background: g.done ? '#22c55e' : '#27272a' }}
              />
            ))}
          </div>
        </div>
        <div className="space-y-2">
          {dailyGoals.map((goal) => (
            <button
              ...
              style={{
                background: goal.done ? 'rgba(34,197,94,0.05)' : '#1a1a1a',
                border: goal.done ? '1px solid rgba(34,197,94,0.15)' : '1px solid transparent',
              }}
            >
              <div
              ...
              <span
                className={`flex-1 text-sm font-bold transition-all ${goal.done ? 'text-zinc-500 line-through' : 'text-white'}`}
              >
                {goal.label}
              </span>
              <div
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${goal.done ? 'border-green-500 bg-green-500' : 'border-zinc-700'}`}
              >
      ...
      {/* Quote */}
      <div
        className="mx-4 mb-6 px-4 py-3 rounded-2xl text-center"
