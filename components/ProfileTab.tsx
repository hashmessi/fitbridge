  ...
  return (
    <div className="p-6 pb-32 min-h-screen animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-white/10 p-6 rounded-[2rem] mb-6 relative overflow-hidden">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-zinc-600 text-center mt-2">
            {xpToNext > 0
              ? `Earn ${xpToNext} more XP to reach ${nextLevel.level} status`
              : 'Max level reached!'}
          </p>
        </div>
      </div>
        ...
        <div className="bg-zinc-900/50 border border-white/5 p-4 rounded-2xl flex items-center justify-between">
          <div>
            ...
            <span className="text-xl font-bold text-white">
              {totalWorkouts} <span className="text-sm font-normal text-zinc-600">total</span>
            </span>
          </div>
        </div>
        <div className="bg-zinc-900/50 border border-white/5 p-4 rounded-2xl flex items-center justify-between">
          <div>
            ...
            <span className="text-xl font-bold text-white">
              {totalMeals} <span className="text-sm font-normal text-zinc-600">total</span>
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 -mx-6 px-6">
          {badges.map((badge) => (
            <div key={badge.id} className="flex flex-col items-center gap-2 min-w-[80px]">
              ...
              <span
                className={`text-[10px] font-bold text-center ${badge.unlocked ? 'text-zinc-300' : 'text-zinc-600'}`}
              >
                {badge.name}
              </span>
            </div>
      ...
      <div className="space-y-3">
        <h3 className="font-bold text-zinc-500 text-xs uppercase tracking-wider pl-1 mb-2">
          Settings
        </h3>
        {[
